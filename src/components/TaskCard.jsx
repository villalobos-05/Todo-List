...
export function TaskCard ({ tasks, cardTitle, changeTask, group, setEditingTask }) {
  return (
    <section className='bg-slate-900 w-[460px] h-fit shadow-[0px_0px_8px_-1px] shadow-black rounded-md p-2 flex flex-col gap-2'>
      ...
      <div onClick={() => setEditingTask({ editing: true, group, id: undefined })} className='flex flex-row w-full gap-1 bg-slate-400/35 hover:bg-slate-400/50 p-2 rounded-md cursor-pointer justify-center transition'>
        <AddIcon />
      </div>
    </section>
  )
